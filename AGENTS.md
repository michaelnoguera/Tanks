...
- Use uv for python. For self-contained plotting scripts use PEP 723 metadata to specify dependencies at the top of the file. Use matplotlib for plotting. Save csv files containing the plotted numbers alongside jpg files of the plots.
For html:
- Use plain HTML wherever possible. Unless otherwise specified, I want single-file HTML. Use dropdowns to hold information that is not immediately necessary to display. Use tables for number-heavy data. Embed graphics inline unless they are stored somewhere standardized already (for example, output plots in ./experiments).
